...
import toppingCacheModel from "../models/toppingsCache";
...
export class OrderController {
  ...
  private calculateTotal = async (cart: CartItem[]) => {
    ...
    const cartToppingIds: any = cart.reduce((acc, item) => {
      return [
        ...acc,
    ...
    const totalPrice = cart.reduce((acc, curr) => {
      const cachedProductPrice: ProductPricingCache = productPricings.find(
        (product) => product.productId === curr._id,
      ) as ProductPricingCache;
